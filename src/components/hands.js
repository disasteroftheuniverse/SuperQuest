...
module.exports = {
   ...
   'hand': AFRAME.registerComponent('oculus-quest-hands', {
      schema: {
         camera: { type: 'selector', default: '#vr-camera' },
         model: { type: 'asset' },
      ...
      onEntityLoaded: function () {
         ...
         if (controller.controller === -1) {
            this.el.addEventListener('controllerconnected', this.onController, { once: true });
         } else {
            this.onController();
         }
      },
      ...
      onModelLoaded: function (animations) {
         ...
         this.animationConfig = { repetitions: 0, time: 0, weight: 0, loop: THREE.LoopRepeat, clampWhenFinished: !0 };
      ...
      registerEventListeners: function () {
         ...
         this.on('triggertouchstart', function (e) {
            this.el.removeState('pointing');
            this.setDirty();
         });

         this.on('triggertouchend', function (e) {
            this.el.addState('pointing');
            this.setDirty();
         });

         this.on('gripdown', function (e) {
            intersections = this.collider.getIntersections('grabbable', true);
            if (intersections) {
               this.holdingEl = intersections.nearestEl;
               this.holdingEl.setAttribute('constraint', { parent: this.el });
         ...
         this.on('constraintchanged', function (e) {
            if (this.el.is('holding-something')) {
         ...
         var onThumbTouchStart = function (e) {
            this.el.removeState('thumbsup');
            this.setDirty();
         };

         var onThumbTouchEnd = function (e) {
