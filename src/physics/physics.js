...
module.exports = {
   system: AFRAME.registerSystem('physics', {
      ...
      __init: async function () {
         ...
         this.emit('init', { WORLD_CONFIG: this.worldConfig });
      },
      emit: async function (evtName, detail) {
         this.__physics.postMessage({ type: evtName, detail: detail });
      },
      ...
      onMessageReceived: async function (evt) {
         this.Engine.emit(evt.data.type, { detail: evt.data.detail });
      },
      registerPhysicsEvents: function () {
         this.Engine.on('physics-started', (e) => {
            this.engineActive = true;
         });
         var sys = this;
         this.Engine.on('post-step', (e) => {
            sys.workerData = e.detail.bodies;
            sys.Bodies.forEach((body, id) => {
               body.setDirty();
            });
         });
