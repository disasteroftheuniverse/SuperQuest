...
var emit = async function(evtName,detail){
   self.postMessage({type: evtName,detail:detail});
};
...
var step = function(detail){
   ...
   emit('post-step',{bodies: bodyDataToPage});
...
self.onmessage = function (evt) {
   data = evt.data; detail = evt.data.detail;
   //console.log(data);
   switch (data.type) {
      case 'init':
         World = new OIMO.World(detail.WORLD_CONFIG);
         World.add({size: [50, 10, 50],pos: [0, -5, 0]});
         emit('physics-started',{msg:'started'} );
         //console.dir(World);
         break;
      case 'step':
         step(detail);
